...
$q = $qb->update('BackBundle:Notification', 'n')
        ->set('n.status', '?1')
...
$p = $q->execute();
	  ...
	  $notifications=$em->getRepository('BackBundle:Notification')->findBy(array("user"=> $currentUser),array('id' => 'desc'),4,0);
 ...
 $notif=$em->getRepository('BackBundle:Notification')->findBy(array("status"=>0,"user"=>$currentUser));
    ...
    public function allnotificationAction(Request $request)
    {
           ...
           $user = $this->container->get('security.token_storage')->getToken()->getUser();
            if((is_object($user) || $user instanceof UserInterface))
            {
             ...
             $notifications =$em->getRepository('BackBundle:Notification')->findBy(array('user'=>$user),array('createdAt' => 'DESC'));
             $notificationpaginator=$this->get('knp_paginator')->paginate(
        $notifications,
        
        $request->query->get('page', 1),8);
             
            return $this->render('NotificationBundle:Default:notifications.html.twig',['notifications'=>$notificationpaginator]); 
    }else{
